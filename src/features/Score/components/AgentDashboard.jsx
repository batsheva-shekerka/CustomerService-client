...
import { useDaylyScoreQuery ,useDaylyimprovetipsQuery,useWeeklyScoreQuery} from '../redux/api';
...
const AgentDashboard = ({ operatorId: propId, name: propName }) => {
  // const o= useSelector((state) => state);
  // סדר עדיפויות ל-ID: קודם prop (מהמנהל), אחרת מה-Redux (המשתמש המחובר)
  const reduxId = useSelector((state) => state.auth?.operator?.operatorId);
  const currentId = propId || reduxId;

  // סדר עדיפויות לשם
  const reduxName = useSelector((state) => state.auth?.operator?.firstName || "נציג/ה");
  const userName = propName || reduxName;
  const { 
    data: tipsData, 
    isLoading: isTipsLoading, 
    isError: isTipsError 
  } = useDaylyimprovetipsQuery(currentId, { skip: !currentId });
  const { 
    data: rawData, 
    isLoading, 
    isError, 
    error 
  } = useDaylyScoreQuery(currentId, { skip: !currentId });
  const { 
    data: weeklyData, 
    isLoading:isWeekLoading, 
    isError:isErrorLoading, 
  } = useWeeklyScoreQuery(currentId, { skip: !currentId });
...
const performanceData = React.useMemo(() => {
  ...
  const latestStats = React.useMemo(() => {
    ...
    return rawData[rawData.length - 1]; // לקיחת הנתון העדכני ביותר
  }, [rawData]);
  console.log("rodata",latestStats)
  console.log("imporvementtips",tipsData)
  console.log("weekaverage",weeklyData)

  if (isLoading|| isTipsLoading||isWeekLoading) return (
      ...
      <div className="container-center">
        ...
        <div className="header-flex">
          <div>
            ...
            <p className="subtitle">שלום {userName}, הנה סיכום הביצועים שלך.</p>
          </div>
  ...
  {tipsData && tipsData.length > 0 ? (
    tipsData.map((tip, index) => {
      // חילוץ הערך המספרי: אם זה אובייקט {impo: 2} או רק המספר 2
      ...
      return (
        <li key={index} style={{ fontSize: '0.875rem', color: '#475569', backgroundColor: '#F8FAFC', padding: '0.75rem', borderRadius: '0.5rem', borderRight: '4px solid #6366f1' }}>
          {/* תרגום הערך המספרי לטקסט, ואם לא קיים - הצגת הערך כפי שהוא */}
