...
import { useGetAllCallsQuery, useDeleteCallMutation ,useGetOperatorCallsQuery} from '../redux/api';
import AddScore from '../../Score/components/AddScore';


const CallsList = ({ readOnly = false, operatorId = null }) => {
// 1. קריאה לכל השיחות - תתבצע רק אם אנחנו לא במצב readOnly
  const { 
    ...
    isError: isErrorAll, 
    error: errorAll 
  } = useGetAllCallsQuery(undefined, { skip: readOnly });
  ...
  const { 
    ...
    isError: isErrorOp, 
    error: errorOp 
  } = useGetOperatorCallsQuery(operatorId, { skip: !readOnly || !operatorId });
  ...
  return (
    <div style={{ padding: '20px' }} dir="rtl">
      ...
      <table border="1" style={{ width: '100%', textAlign: 'right', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ backgroundColor: '#f2f2f2' }}>
            ...
            <th>תמלול טלפנית</th>
            <th>רגש טלפנית</th>
            <th>וויליום מקסימלי טלפנית</th>
            <th>מילים לשניה טלפנית</th>
            <th>תמלול לקוח</th>
            <th>רגש לקוח התחלה</th>
            <th>רגש לקוח סוף</th>
            <th>וויליום מקסימלי לקוח</th>
            <th>הערות שיחה</th>
        ...
        <tbody>
          {list?.map((call, index) => {
            ...
            const OperatorId = call.operatorId;
            const OperatorName=call.operatorName;
            const CallId =call.callId;
            const CompanyId=call.companyId;
            const OperatorTranscript=call.operatorTranscript
            const OperatorSentiment=call.operatorSentiment;
            const OperatorMaxVolume=call.operatorMaxVolume;
            const OperatorWordsPerSecond=call.operatorWordsPerSecond;
            const CustomerTranscript=call.customerTranscript;
            const CustomerSentimentStart=call.customerSentimentStart;
            const CustomerSentimentEnd=call.customerSentimentEnd;
            const CustomerMaxVolume=call.customerMaxVolume;
            const GeneralNotes=call.generalNotes;
            ...
            return (
              <tr key={CallId || index}>
                ...
                <td>{OperatorName}</td>
                <td>{CompanyId}</td>
                <td>{OperatorTranscript}</td>
                <td>{OperatorSentiment}</td>
                <td>{OperatorMaxVolume}</td>
                <td>{OperatorWordsPerSecond}</td>
                <td>{CustomerTranscript}</td>
                <td>{CustomerSentimentStart}</td>
                <td>{CustomerSentimentEnd}</td>
                <td>{CustomerMaxVolume}</td>
                <td>{GeneralNotes}</td>
                {!readOnly&&  
                <td >
                  <Button color="primary" onClick={() => showScore(call)}>ציון לשיחה</Button>
      ...
      <AddCall 
        ...
        callToEdit={selectedCall?.CallId}
      />
     <AddScore 
        open={isDialogOpen} 
        handleClose={handleCloseDialog} 
